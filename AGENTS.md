...
- **Conversation pi** — runs on `main` (this checkout). Owns dialogue, planning (PRD/contract/task docs and `docs/tasks/STATUS.md`), project setup (new and existing), worktree dispatch, acceptance review, and all git merges/releases. May perform any git operation across all branches with the user's explicit permission; advancing `main` always needs that approval. Never writes business code. May edit process/rule and planning docs on `main-relay`.
- **Executor pi** — runs on a `<task>` branch. Implements one task, reports command + result, and **must never merge** (not into `main-relay`, not into `main`).
...
- The conversation pi writes task documents on `main-relay`; an executor reads its assigned task doc and must not rewrite it without approval.
...
- Do not merge your task branch (never into `main-relay` or `main`). Report command + result and let the conversation pi review and merge.
