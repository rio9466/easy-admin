...
- 两个角色分工（对话 / 执行），各自钉在固定分支上，职责不重叠。
- 所有协作围绕**文档**进行：PRD → 契约 → 任务文档 → 提交与验收证据。
- 发布：`main-relay` 合入 `main`，打标签。

## 2. 分支模型

| 分支 | 作用 | 谁能动 |
| ---- | ---- | ------ |
| `main` | 发布基线，只读 | 只有“对话 pi”，且需用户明确同意才合并 |
| `main-relay` | AI 集成分支（由对话 pi 管理） | 对话 pi（任务合回这里） |
| `<task>` | 每个任务一条临时分支 | 执行 pi |

规则：新任务分支**只从 `main-relay` 切**，合回 `main-relay`；不直接提交 `main`。

## 3. 两个角色

原「编排/规划 pi」已并入对话 pi，不再有独立的 relay pi；`main-relay` 仍是集成分支（由对话 pi 管理）。

- **对话 pi**（跑在 `main` 主检出）：对话/决策、**规划（PRD/契约/任务拆分、维护台账）**、
  **项目初始化与老项目接入**、worktree 派发、独立验收、所有 git 合并与发布。
  不写业务代码。持有**本地持久记忆**（见 §6）。用户允许时可做任意 git 操作。
- **执行 pi**（跑在任务分支）：只实现一个任务、自测、回报「命令 + 结果」。
  不改契约/跨领域文档，**永不合并**（既不进 `main-relay` 也不进 `main`）。
...
| `docs/adr/` | 架构决策记录（ADR） | 主线 |
| `docs/api/*.md` | 前后端 API 契约说明（源码真相在本项目为 `server/docs/openapi.yaml`） | 主线 |
...
1. **建仓库**：`main` 为默认分支；推送到远端。
2. **抄治理文档**：复制 `AGENTS.md`、`ORCA_WORKFLOW.md`、`.gitignore`，以及 `docs/` 骨架
   （`docs/README.md`、`docs/prd/`、`docs/adr/`、`docs/api/`、`docs/tasks/README.md` + `STATUS.md`）。
...
> **区域按你的项目定义；下面是本项目（easy-admin-main）的区域示例。**

| 示例区域 | 技术栈 | 说明 |
| -------- | ------ | ---- |
| `server/` | Go API 服务 | 后端服务，含 `server/docs/openapi.yaml` 契约 |
| `server/admin/` | Vue 3 + Element Plus + pnpm | 管理后台 |
...
- **端口**：不同环境/模板用不同端口，避免开发分支互相占用；本地运行配置（`config.local.toml`、
  `configs/*.local.env`、`.env*`）一律 git-ignore，只提交 `.example`。
  本项目参考：服务 `8080`、管理后台 `8848`。
- **契约优先**：以一份契约文档为准；执行者不得私改请求/响应形状，发现缺口先报告。
  本项目契约真相为 `server/docs/openapi.yaml`（管理后台用 `pnpm generate:api` 生成类型）。
- **验收**：执行者必须给「命令 + 结果」，对话 pi 独立复核后才合并。
- **发布**：`main-relay` → `main`，打 `vX.Y.Z` 标签。
